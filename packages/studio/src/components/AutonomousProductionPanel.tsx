...
export function AutonomousProductionCard({ view, pending, error, onStart, onStop, onRepair, onAbandon, onConfigureModels, language = "en" }: {
  ...
  return <section data-testid="autonomous-production" className="paper-sheet rounded-2xl border border-primary/20 p-5 shadow-sm space-y-4">
    ...
    {view.economics.currentAttempt && <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4"><div><span className="text-muted-foreground">{tr("Logical Calls", "逻辑调用")}</span><div className="font-semibold">{view.economics.currentAttempt.logicalCalls}</div></div><div><span className="text-muted-foreground">{tr("Provider Transports", "Provider 传输")}</span><div className="font-semibold">{view.economics.currentAttempt.providerTransports}</div></div><div><span className="text-muted-foreground">{tr("Tokens", "令牌")}</span><div className="font-semibold">{view.economics.currentAttempt.totalTokens}</div></div><div><span className="text-muted-foreground">{tr("Estimated Cost", "估算费用")}</span><div className="font-semibold">{localizedMoney(view.economics.currentAttempt.estimatedCostUsd)}</div></div></div>}
    ...
    {!repair && !missingModels && !repairNeedsReconciliation && <div className="space-y-2"><div className="flex flex-wrap gap-2"><button disabled={!view.startEnabled || pending || waiting} onClick={() => onStart("current-volume")} className="h-9 rounded-lg bg-primary px-4 text-sm font-bold text-primary-foreground disabled:opacity-40">{tr("Resume", "继续")}</button>{active && !waiting && <button disabled={pending} onClick={onStop} className="h-9 rounded-lg border px-4 text-sm font-bold">{tr("Stop", "停止")}</button>}</div></div>}
    {(error || view.runtime?.lastError) && <p className="text-sm text-destructive">{tr("Production needs attention. See Advanced Details.", "生产需要处理。请查看高级详情。")}</p>}
    <details className="rounded-xl border border-border/50 p-3 text-xs"><summary className="cursor-pointer font-semibold">{tr("Advanced Details", "高级详情")}</summary><div className="mt-3 space-y-2 text-muted-foreground"><p>{tr("Raw status", "原始状态")}: {view.runtimeStatus} · {tr("phase", "阶段")}: {view.runtime?.phase ?? "none"} · {tr("role", "角色")}: {view.runtime?.activeRole ?? "none"} · Provider: {view.runtime?.activeProvider ?? "none"} · {tr("model", "模型")}: {view.runtime?.activeModel ?? "none"}</p>{view.runtime?.attempt !== undefined && <p>{tr("Transport attempt", "传输尝试")}: {view.runtime.attempt} / {view.runtime.maxAttempts ?? 3} · logicalStepId: {view.runtime.logicalStepId ?? "Unavailable"} · {tr("response artifact", "响应证据")}: {view.runtime.responseArtifactStatus ?? "NONE"}</p>}<p>{tr("Historical calls", "历史调用")}: {view.economics.actual.providerCalls} · {tr("historical tokens", "历史 Token")}: {view.economics.actual.totalTokens}</p>{view.economics.currentAttempt && <p>{tr("Current token discrepancy", "当前 Token 差异")}: {view.economics.currentAttempt.tokenDiscrepancy} · {tr("Provider actual", "服务商实际费用")}: {money(view.economics.currentAttempt.actualCostUsd)}</p>}{view.economics.currentAttempt?.integrityWarnings?.map((warning) => <p key={warning}>{tr("Telemetry warning", "遥测警告")}: {warning}</p>)}<p>{tr("Roles", "角色")}: {Object.entries(view.roles).map(([r,m]) => `${r}=${m ?? "NOT_CONFIGURED"}`).join(" · ")}</p>{view.runtimeBlockers.map((b) => <p key={b}>{humanBlocker(b)} <code>({b})</code></p>)}{(error || view.runtime?.lastError) && <p>{tr("Exception", "异常")}: {error ?? view.runtime?.lastError}</p>}</div></details>
  </section>;
}
