...
const resumePreservedBoundedCandidateReviewMock = vi.fn();
const reconcilePreservedBoundedCandidateTerminalMock = vi.fn();
...
const listBooksMock = vi.fn<() => Promise<string[]>>();
...
vi.mock("@actalk/inkos-core", async (importOriginal) => {
  ...
  actualResolveFormalPendingChapterRecoveryPlan = actual.resolveFormalPendingChapterRecoveryPlan as typeof actualResolveFormalPendingChapterRecoveryPlan;
  ...
  class MockPipelineRunner {
    ...
    resumePreservedBoundedCandidateReview = resumePreservedBoundedCandidateReviewMock;
    reconcilePreservedBoundedCandidateTerminal = reconcilePreservedBoundedCandidateTerminalMock;
  ...
  return {
    ...
    refreshAutonomousJobClaim: actual.refreshAutonomousJobClaim,
    releaseAutonomousJob: actual.releaseAutonomousJob,
    startAutonomousJobHeartbeat: actual.startAutonomousJobHeartbeat,
    autonomousProductionStatePath: actual.autonomousProductionStatePath,
    loadAutonomousProductionState: actual.loadAutonomousProductionState,
    runBoundedAutonomousScope: actual.runBoundedAutonomousScope,
    saveAutonomousProductionState: actual.saveAutonomousProductionState,
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    ...
    resumePreservedBoundedCandidateReviewMock.mockReset();
    reconcilePreservedBoundedCandidateTerminalMock.mockReset();
  ...
  it("routes one Studio start through the shared Core controller, reuses an audit-failed draft, and persists the dynamic-volume checkpoint", async () => {
    ...
    await writeFile(join(root, "books", "demo-book", "story", "outline", "book-production-map.json"), JSON.stringify({
      ...
      volumes: [
        { volume_id: "volume-001", volume_number: 1, title: "One", start_chapter: 1, end_chapter: 5, chapter_count: 5 },
        { volume_id: "volume-002", volume_number: 2, title: "Two", start_chapter: 6, end_chapter: 7, chapter_count: 2 },
      ],
    }), "utf-8");
    loadSecretsMock.mockResolvedValue({ services: { openrouter: { apiKey: "test-only-secret" } } });
    probeModelsFromUpstreamMock.mockResolvedValue(["openai/gpt", "deepseek/chat", "google/gemini", "deepseek/flash"].map((id) => ({
    ...
    let chapters = [1, 2, 3, 4].map((number) => ({
      number, title: `Chapter ${number}`, status: number === 4 ? "audit-failed" : "approved", wordCount: 1,
      auditIssues: number === 4 ? ["[warning] persisted finding"] : [], lengthWarnings: [],
    ...
    resumeAuditFailedChapterBoundedMock.mockImplementationOnce(() => new Promise((resolve) => {
      releaseResume = () => resolve({ chapterNumber: 4, status: "approved", revisionCount: 1, logicReviewCount: 1, commercialReviewCount: 1, roleUsage: {} });
    }));
    writeNextChapterMock.mockImplementationOnce(async () => {
      chapters = [...chapters, { number: 5, title: "Chapter 5", status: "ready-for-review", wordCount: 1, auditIssues: [], lengthWarnings: [], tokenUsage: { promptTokens: 1, completionTokens: 1, totalTokens: 2 } }];
      nextChapter = 6;
    ...
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      status: "READY", mode: "current-volume", nextChapter: 5, updatedAt: "2026-08-21T00:00:00.000Z",
      repairOutcome: { chapter: 4, status: "STATE_REPAIRED_REVIEW_STILL_REQUIRED", errorCode: null },
    }), "utf-8");

    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    const first = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production/start", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "current-volume" }),
    });
    const second = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production/start", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "current-volume" }),
    });
    expect(first.status).toBe(202);
    expect(second.status).toBe(409);
    ...
    await vi.waitFor(async () => {
      const runtime = JSON.parse(await readFile(join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous", "production-state.json"), "utf-8"));
      expect(runtime).toMatchObject({ status: "VOLUME_COMPLETE", nextChapter: 6, budget: { status: "BUDGET_NOT_CONFIGURED" } });
      expect(runtime.jobId).toMatch(/^autonomous-[a-f0-9]{32}$/);
      expect(runtime.volumeCostSettlement).toMatchObject({ volumeId: "volume-001", startChapter: 1, endChapter: 5, budgetStatus: "BUDGET_NOT_CONFIGURED" });
      expect(runtime.volumeCostSettlement.currentVolumeCalculatedCostUsd).toBeGreaterThan(0);
  ...
  it("preserves rebaseline ownership and blocks a third attempt after double state validation failure", async () => {
    ...
    await writeFile(join(root, "inkos.json"), JSON.stringify(raw, null, 2), "utf-8");
    const map = {
      schema_version: "1.0", book_id: "demo-book", authority_book_id: "authority", title: "Demo", total_chapters: 5,
      volumes: [{ volume_id: "volume-001", volume_number: 1, title: "One", start_chapter: 1, end_chapter: 5, chapter_count: 5 }],
    };
    await mkdir(join(root, "books", "demo-book", "story", "outline"), { recursive: true });
    await writeFile(join(root, "books", "demo-book", "story", "outline", "book-production-map.json"), JSON.stringify(map), "utf-8");
    loadSecretsMock.mockResolvedValue({ services: { openrouter: { apiKey: "test-only-secret" } } });
    probeModelsFromUpstreamMock.mockResolvedValue(["openai/gpt", "deepseek/chat", "google/gemini", "deepseek/flash"].map((id) => ({
      id, name: id, contextWindow: 128_000, maxOutputTokens: 16_000,
      inputPrice: "0.000001", outputPrice: "0.000004", inputModalities: ["text"], outputModalities: ["text"],
    })));
    loadBookConfigMock.mockResolvedValue({ id: "demo-book", title: "Demo", genre: "urban", targetChapters: 5 });
    let nextChapter = 5;
    getNextChapterNumberMock.mockImplementation(async () => nextChapter);
    let chapters = [1, 2, 3, 4].map((number) => ({
      number, title: `Chapter ${number}`, status: number === 4 ? "audit-failed" : "approved", wordCount: 1,
    ...
    rebaselinePendingChapterStateMock.mockImplementationOnce(async (plan) => {
      ...
      throw new Error("STATE_REBASELINE_VALIDATION_FAILED");
    });
    const runtimeDir = join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous");
    await mkdir(runtimeDir, { recursive: true });
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      jobId, status: "REVIEW_EXHAUSTED", mode: "current-volume", volumeId: "volume-001",
      startChapter: 4, targetChapter: 5, nextChapter: 5, chapterNumber: 5, completedThisRun: 0,
      responseArtifactStatus: "COMPLETE", revisionRound: 2, updatedAt: "2026-08-23T00:00:00.000Z",
    }), "utf-8");

    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    const response = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production/start", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "current-volume" }),
    });
    ...
    const view = await viewResponse.json() as { startEnabled: boolean; runtimeBlockers: string[] };
    expect(view.startEnabled).toBe(false);
    expect(view.runtimeBlockers).toContain("STATE_REBASELINE_VALIDATION_FAILED");
    const resumeResponse = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production/start", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "current-volume" }),
    });
    expect(resumeResponse.status).toBe(409);
    expect(rebaselinePendingChapterStateMock).toHaveBeenCalledTimes(1);
    expect(resumeAuditFailedChapterBoundedMock).not.toHaveBeenCalled();
    expect(writeNextChapterMock).not.toHaveBeenCalled();
  });

  it.each([
    { decision: "APPROVED" as const, pipelineStatus: "ready-for-review" as const, expectedIndexStatus: "approved" as const },
    { decision: "ACCEPTED_WITH_FINDINGS" as const, pipelineStatus: "accepted-with-findings" as const, expectedIndexStatus: "accepted-with-findings" as const },
  ])("terminalizes a generic preserved Chapter 6 before advancing to Chapter 7 ($decision)", async ({ decision, pipelineStatus, expectedIndexStatus }) => {
    const raw = JSON.parse(await readFile(join(root, "inkos.json"), "utf-8"));
    raw.llm = { ...raw.llm, service: "openrouter", defaultModel: "openai/gpt", model: "openai/gpt", services: [{ service: "openrouter" }] };
    raw.modelOverrides = { auditor: "deepseek/chat", "commercial-reader": "google/gemini", reviser: "openai/gpt", "observer-reflector": "deepseek/flash" };
    await writeFile(join(root, "inkos.json"), JSON.stringify(raw, null, 2), "utf-8");
    ...
    await writeFile(join(root, "books", "demo-book", "story", "outline", "book-production-map.json"), JSON.stringify({
      schema_version: "1.0", book_id: "demo-book", authority_book_id: "authority", title: "Demo", total_chapters: 7,
      volumes: [
        { volume_id: "volume-001", volume_number: 1, title: "One", start_chapter: 1, end_chapter: 6, chapter_count: 6 },
        { volume_id: "volume-002", volume_number: 2, title: "Two", start_chapter: 7, end_chapter: 7, chapter_count: 1 },
      ],
    }));
    loadSecretsMock.mockResolvedValue({ services: { openrouter: { apiKey: "test-only-secret" } } });
    probeModelsFromUpstreamMock.mockResolvedValue(["openai/gpt", "deepseek/chat", "google/gemini", "deepseek/flash"].map((id) => ({
    ...
    let nextChapter = 6;
    getNextChapterNumberMock.mockImplementation(async () => nextChapter);
    let chapters = [1, 2, 3, 4, 5].map((number) => ({
    ...
    const { deriveAutonomousJobIdentity, loadBookProductionMap } = await import("@actalk/inkos-core");
    const normalizedMap = await loadBookProductionMap(root, "demo-book");
    const jobId = deriveAutonomousJobIdentity({ map: normalizedMap!, mode: "current-volume", nextChapter: 6 });
    const admittedPlan = {
      kind: "FORMAL_PRESERVED_BOUNDED_REVIEW_RESUME", recoveryClass: "PRESERVED_BOUNDED_REVIEW",
      bookId: "demo-book", jobId, pendingChapterNumber: 6, baselineChapterNumber: 5,
      ...
      initialReviews: { "logic-canon-auditor": { reviewerRole: "logic-canon-auditor", decision: "REVISION_REQUIRED" } },
      invalidReviewerRoles: ["commercial-reader"], historicalRoleUsage: {},
    };
    resolveFormalPendingChapterRecoveryPlanMock.mockResolvedValue(admittedPlan);
    resumePreservedBoundedCandidateReviewMock.mockImplementation(async (plan) => {
      expect(plan).toBe(admittedPlan);
      const bookDir = join(root, "books", "demo-book");
      await writeFile(join(bookDir, "chapters", "0006_Generic_Six.md"), "# Chapter 6: Generic Six\n\nCANDIDATE_A");
      await mkdir(join(bookDir, "story", "snapshots", "6", "state"), { recursive: true });
      await writeFile(join(bookDir, "story", "snapshots", "6", "state", "manifest.json"), JSON.stringify({ schemaVersion: 2, lastAppliedChapter: 6 }));
      chapters = [...chapters, {
        number: 6, title: "Generic Six", status: pipelineStatus, wordCount: 1, auditIssues: [], lengthWarnings: [],
      }];
      nextChapter = 7;
      return { chapterNumber: 6, status: pipelineStatus, autonomousReview: { status: decision } };
    });
    const runtimeDir = join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous");
    await mkdir(runtimeDir, { recursive: true });
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      jobId, status: "HELD_AFTER_TWO_REVISIONS", mode: "current-volume", volumeId: "volume-001",
      startChapter: 6, targetChapter: 6, nextChapter: 6, chapterNumber: 6, completedThisRun: 0,
      responseArtifactStatus: "COMPLETE", updatedAt: "2026-08-27T00:00:00.000Z",
    }));
    ...
    await vi.waitFor(() => expect(resumePreservedBoundedCandidateReviewMock).toHaveBeenCalledTimes(1));
    expect(writeNextChapterMock).not.toHaveBeenCalled();
    expect(resumeAuditFailedChapterBoundedMock).not.toHaveBeenCalled();
    await vi.waitFor(async () => {
      const runtime = JSON.parse(await readFile(join(runtimeDir, "production-state.json"), "utf-8"));
      expect(runtime).toMatchObject({ status: "VOLUME_COMPLETE", nextChapter: 7, recoveryOwnership: null });
      expect(chapters.find((chapter) => chapter.number === 6)?.status).toBe(expectedIndexStatus);
      expect(chapters.find((chapter) => chapter.number === 6)?.status).not.toBe("ready-for-review");
      await expect(access(join(root, "books", "demo-book", "chapters", "0006_Generic_Six.md"))).resolves.toBeUndefined();
      await expect(access(join(root, "books", "demo-book", "story", "snapshots", "6", "state", "manifest.json"))).resolves.toBeUndefined();
      await expect(access(join(runtimeDir, "active-job.json"))).rejects.toMatchObject({ code: "ENOENT" });
      await expect(readdir(join(runtimeDir, "active-job.json.reclaim-contenders"))).resolves.toEqual([]);
    });
  });

  it("discovers a generic terminal length violation behind descendants and resumes it without creative Writer generation", async () => {
    const raw = JSON.parse(await readFile(join(root, "inkos.json"), "utf-8"));
    raw.llm = { ...raw.llm, service: "openrouter", defaultModel: "openai/gpt", model: "openai/gpt", services: [{ service: "openrouter" }] };
    raw.modelOverrides = { auditor: "deepseek/chat", "commercial-reader": "google/gemini", reviser: "openai/gpt", "observer-reflector": "deepseek/flash" };
    await writeFile(join(root, "inkos.json"), JSON.stringify(raw, null, 2), "utf-8");
    await mkdir(join(root, "books", "demo-book", "story", "outline"), { recursive: true });
    await writeFile(join(root, "books", "demo-book", "story", "outline", "book-production-map.json"), JSON.stringify({
      schema_version: "1.0", book_id: "demo-book", authority_book_id: "authority", title: "Demo", total_chapters: 8,
      volumes: [{ volume_id: "volume-001", volume_number: 1, title: "One", start_chapter: 1, end_chapter: 8, chapter_count: 8 }],
    }));
    loadSecretsMock.mockResolvedValue({ services: { openrouter: { apiKey: "test-only-secret" } } });
    probeModelsFromUpstreamMock.mockResolvedValue(["openai/gpt", "deepseek/chat", "google/gemini", "deepseek/flash"].map((id) => ({
      id, name: id, contextWindow: 128_000, maxOutputTokens: 16_000, inputPrice: "0.000001", outputPrice: "0.000004", inputModalities: ["text"], outputModalities: ["text"],
    })));
    loadBookConfigMock.mockResolvedValue({ id: "demo-book", title: "Demo", genre: "urban", targetChapters: 8 });
    let nextChapter = 8;
    getNextChapterNumberMock.mockImplementation(async () => nextChapter);
    loadChapterIndexMock.mockResolvedValue([1, 2, 3, 4, 5, 6, 7].map((number) => ({
      number, title: `Chapter ${number}`, status: number === 7 ? "state-degraded" : "approved", wordCount: number === 6 ? 700 : 2000, auditIssues: [], lengthWarnings: [],
    })));
    const { deriveAutonomousJobIdentity, loadBookProductionMap } = await import("@actalk/inkos-core");
    const normalizedMap = await loadBookProductionMap(root, "demo-book");
    const jobId = deriveAutonomousJobIdentity({ map: normalizedMap!, mode: "current-volume", nextChapter: 6 });
    const plan = {
      ...
      bookId: "demo-book", jobId, pendingChapterNumber: 6, baselineChapterNumber: 5, productionMapSha256: "a".repeat(64),
    ...
    resolveFormalPendingChapterRecoveryPlanMock.mockImplementation(async ({ pendingChapterNumber }: { pendingChapterNumber: number }) => pendingChapterNumber === 6 ? plan : null);
    resumePreservedBoundedCandidateReviewMock.mockImplementation(async () => {
      nextChapter = 6;
      return { chapterNumber: 6, status: "blocked-critical-findings", autonomousReview: { status: "BLOCKED_CRITICAL_FINDINGS" } };
    });
    const runtimeDir = join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous");
    await mkdir(runtimeDir, { recursive: true });
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      jobId, status: "PAUSED_BY_USER", mode: "current-volume", volumeId: "volume-001", startChapter: 6, targetChapter: 8,
      nextChapter: 8, chapterNumber: 7, completedThisRun: 2, responseArtifactStatus: "COMPLETE", updatedAt: "2026-08-28T00:00:00.000Z",
    }));

    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    const projection = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production");
    expect(projection.status).toBe(200);
    expect(await projection.json()).toMatchObject({
      runtimeStatus: "RECOVERY_READY_PRESERVED_BOUNDED_REVIEW", startEnabled: true,
      finalReviewRecovery: { chapter: 6, recoveryClass: "TERMINAL_LENGTH_VIOLATION", downstreamAction: "DISCARD_AND_REGENERATE", additionalWriterCalls: 0 },
    });
    const response = await app.request("http://localhost/api/v1/books/demo-book/autonomous-production/start", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "current-volume" }),
    });
    expect(response.status, await response.clone().text()).toBe(202);
    await vi.waitFor(() => expect(resumePreservedBoundedCandidateReviewMock).toHaveBeenCalledWith(plan));
    expect(writeNextChapterMock).not.toHaveBeenCalled();
    await vi.waitFor(async () => expect(JSON.parse(await readFile(join(runtimeDir, "production-state.json"), "utf-8")))
      .toMatchObject({ status: "BLOCKED_CRITICAL_FINDINGS", nextChapter: 6, recoveryOwnership: { recoveryClass: "TERMINAL_LENGTH_VIOLATION" } }));
  });

  it("reconciles a terminal-length recovered Chapter 6 after restart without rerunning review or generation", async () => {
    const raw = JSON.parse(await readFile(join(root, "inkos.json"), "utf-8"));
    raw.llm = { ...raw.llm, service: "openrouter", defaultModel: "openai/gpt", model: "openai/gpt", services: [{ service: "openrouter" }] };
    raw.modelOverrides = { auditor: "deepseek/chat", "commercial-reader": "google/gemini", reviser: "openai/gpt", "observer-reflector": "deepseek/flash" };
    await writeFile(join(root, "inkos.json"), JSON.stringify(raw, null, 2), "utf-8");
    ...
    loadBookConfigMock.mockResolvedValue({ id: "demo-book", title: "Demo", genre: "urban", targetChapters: 7 });
    getNextChapterNumberMock.mockResolvedValue(7);
    loadChapterIndexMock.mockResolvedValue([1, 2, 3, 4, 5, 6].map((number) => ({
      number, title: `Chapter ${number}`, status: "approved", wordCount: 1, auditIssues: [], lengthWarnings: [],
    })));
    const { deriveAutonomousJobIdentity, loadBookProductionMap } = await import("@actalk/inkos-core");
    const normalizedMap = await loadBookProductionMap(root, "demo-book");
    const jobId = deriveAutonomousJobIdentity({ map: normalizedMap!, mode: "current-volume", nextChapter: 6 });
    resolveFormalPendingChapterRecoveryPlanMock.mockResolvedValue({
      kind: "FORMAL_PRESERVED_BOUNDED_REVIEW_RESUME", recoveryClass: "TERMINAL_LENGTH_VIOLATION",
      bookId: "demo-book", jobId, pendingChapterNumber: 6, baselineChapterNumber: 5,
      productionMapSha256: "a".repeat(64),
      candidate: { content: "ORIGINAL", sha256: "b".repeat(64), title: "Generic Six", titleAuthorityLogicalStepId: `provider-step-${"c".repeat(64)}`, titleAuthorityArtifactSha256: "d".repeat(64) },
      reviewEvidence: { relativePath: "review.json", sha256: "e".repeat(64), runRelativePath: "run.json", runSha256: "f".repeat(64) },
      initialReviews: {}, invalidReviewerRoles: ["commercial-reader"], historicalRoleUsage: {},
      terminalLengthRecovery: {
        bookConfigSha256: "5".repeat(64), baselineSnapshotFiles: [], currentChapterFile: "0006_Generic_Six.md",
        currentChapterSha256: "6".repeat(64), currentLengthCount: 700, candidateLengthCount: 2200,
        terminalReceiptRelativePath: "old-terminal.json", terminalReceiptSha256: "7".repeat(64), terminalCandidateSha256: "8".repeat(64),
        hardMin: 1600, hardMax: 2800, downstreamChapters: [], preparationStatus: "REWOUND",
        preparationReceiptRelativePath: "rewound.json", preparationReceiptSha256: "9".repeat(64),
      },
      terminalReconciliation: {
        status: "approved", chapterFile: "0006_Generic_Six.md", candidateSha256: "1".repeat(64),
        receiptRelativePath: "story/runtime/bounded-autonomous/chapter-0006/preserved-review-resume-001.json",
        receiptSha256: "2".repeat(64), snapshotManifestSha256: "3".repeat(64), stateManifestSha256: "4".repeat(64),
      },
    });
    reconcilePreservedBoundedCandidateTerminalMock.mockResolvedValue({ chapterNumber: 6, status: "approved" });
    const runtimeDir = join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous");
    await mkdir(runtimeDir, { recursive: true });
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      ...
      recoveryOwnership: {
        kind: "FORMAL_PRESERVED_BOUNDED_REVIEW_RESUME", recoveryClass: "TERMINAL_LENGTH_VIOLATION",
        bookId: "demo-book", jobId, pendingChapterNumber: 6,
      },
    }));
    ...
    expect(resumePreservedBoundedCandidateReviewMock).not.toHaveBeenCalled();
    expect(reconcilePreservedBoundedCandidateTerminalMock).toHaveBeenCalledTimes(1);
    expect(writeNextChapterMock).not.toHaveBeenCalled();
    expect(resumeAuditFailedChapterBoundedMock).not.toHaveBeenCalled();
    expect(rebaselinePendingChapterStateMock).not.toHaveBeenCalled();
    expect(finalizePendingChapterOfflineMock).not.toHaveBeenCalled();
  });

  it("re-enters non-Chapter-004 bounded state rebaseline ownership after a durable Provider wait and Studio restart", async () => {
    const raw = JSON.parse(await readFile(join(root, "inkos.json"), "utf-8"));
    raw.llm = { ...raw.llm, service: "openrouter", defaultModel: "openai/gpt", model: "openai/gpt", services: [{ service: "openrouter" }] };
    raw.modelOverrides = { auditor: "deepseek/chat", "commercial-reader": "google/gemini", reviser: "openai/gpt", "observer-reflector": "deepseek/flash" };
    await writeFile(join(root, "inkos.json"), JSON.stringify(raw, null, 2), "utf-8");
    ...
    let nextChapter = 7;
    getNextChapterNumberMock.mockImplementation(async () => nextChapter);
    let chapters = [1, 2, 3, 4, 5, 6].map((number) => ({
      number, title: `Chapter ${number}`, status: number === 6 ? "audit-failed" : "approved", wordCount: 1,
      auditIssues: number === 6 ? ["[warning] persisted finding"] : [], lengthWarnings: [],
      tokenUsage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
    }));
    loadChapterIndexMock.mockImplementation(async () => chapters);
    saveChapterIndexMock.mockImplementation(async (_bookId, updated) => { chapters = updated; });
    writeNextChapterMock.mockImplementationOnce(async () => {
      chapters = [...chapters, { number: 7, title: "Chapter 7", status: "ready-for-review", wordCount: 1, auditIssues: [], lengthWarnings: [], tokenUsage: { promptTokens: 1, completionTokens: 1, totalTokens: 2 } }];
      nextChapter = 8;
      return { chapterNumber: 7, status: "ready-for-review" };
    });
    const { createAutonomousProviderExecution, deriveAutonomousJobIdentity, loadBookProductionMap } = await import("@actalk/inkos-core");
    const normalizedMap = await loadBookProductionMap(root, "demo-book");
    const jobId = deriveAutonomousJobIdentity({ map: normalizedMap!, mode: "current-volume", nextChapter: 7 });
    const admittedPlan = {
      kind: "FORMAL_BOUNDED_STATE_REBASELINE",
      recoveryClass: "ORIGINAL_REVIEW_EXHAUSTED",
      bookId: "demo-book",
      ...
      baselineChapterNumber: 5,
      finalReview: { decision: "APPROVED" },
    };
    resolveFormalPendingChapterRecoveryPlanMock.mockResolvedValue(admittedPlan);
    const replayStage = { stage: "STATE_REBASELINE_VALIDATE", role: "state-validator", provider: "openrouter", model: "deepseek/flash" } as const;
    const replayExecution = createAutonomousProviderExecution({
      projectRoot: root, bookId: "demo-book", jobId, getActiveStage: () => replayStage,
    });
    const replayFingerprint = "7".repeat(64);
    const replayPath = replayExecution.responseArtifactPath(replayFingerprint, "openrouter", "deepseek/flash", 6);
    const replayLogicalStepId = replayPath.split(/[\\/]/).at(-1)!.replace(/\.json$/u, "");
    const replayResponse = { content: "persisted state validation", usage: { promptTokens: 3, completionTokens: 2, totalTokens: 5 } };
    const { createHash } = await import("node:crypto");
    const runtimeDir = join(root, "books", "demo-book", "story", "runtime", "bounded-autonomous");
    await mkdir(join(runtimeDir, "provider-responses"), { recursive: true });
    await writeFile(replayPath, `${JSON.stringify({
      schema_version: "1.0", job_id: jobId, logical_step_id: replayLogicalStepId, usage_identity: replayLogicalStepId,
      chapter_number: 6, role: replayStage.role, stage: replayStage.stage, provider: "openrouter", requested_model: "deepseek/flash",
      input_fingerprint: replayFingerprint, response_artifact_status: "COMPLETE",
      content_sha256: createHash("sha256").update(replayResponse.content).digest("hex"), response: replayResponse,
      completed_at: "2026-08-27T00:00:00.000Z",
    }, null, 2)}\n`, "utf-8");
    let paidTransportCalls = 0;
    rebaselinePendingChapterStateMock.mockImplementation(async () => {
      await replayExecution.runProviderCall(6, async () => {
        paidTransportCalls += 1;
        return { content: "must not run", usage: { promptTokens: 1, completionTokens: 1, totalTokens: 2 } };
      }, { provider: "openrouter", model: "deepseek/flash", inputFingerprint: replayFingerprint });
      chapters = chapters.map((chapter) => chapter.number === 6 ? { ...chapter, status: "approved" } : chapter);
      return { chapterNumber: 6, status: "approved", revisionCount: 2, logicReviewCount: 2, commercialReviewCount: 0, roleUsage: {} };
    });
    await mkdir(runtimeDir, { recursive: true });
    await writeFile(join(runtimeDir, "production-state.json"), JSON.stringify({
      jobId,
      ...
      logicalStepId: "provider-step-restart-test",
      chapterNumber: 6,
      role: "logicAuditor",
      stage: "REVISION_AND_LOGIC",
      provider: "openrouter",
    ...
    listBooksMock.mockResolvedValue(["demo-book"]);

    const { createStudioServer } = await import("./server.js");
    createStudioServer(cloneProjectConfig() as never, root);

    await vi.waitFor(() => expect(rebaselinePendingChapterStateMock).toHaveBeenCalledWith(admittedPlan));
    expect(resumeAuditFailedChapterBoundedMock).not.toHaveBeenCalled();
    await vi.waitFor(() => expect(writeNextChapterMock).toHaveBeenCalledTimes(1));
    await vi.waitFor(async () => {
      const runtime = JSON.parse(await readFile(join(runtimeDir, "production-state.json"), "utf-8"));
      expect(runtime).toMatchObject({ jobId, status: "BOOK_COMPLETE", nextChapter: 8, recoveryOwnership: null });
      await expect(access(join(runtimeDir, "active-job.json"))).rejects.toMatchObject({ code: "ENOENT" });
      await expect(readdir(join(runtimeDir, "active-job.json.reclaim-contenders"))).resolves.toEqual([]);
    });
    expect(rebaselinePendingChapterStateMock).toHaveBeenCalledTimes(1);
    expect(paidTransportCalls).toBe(0);
  });
